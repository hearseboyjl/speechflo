export default function SettingsModal({ open, onClose, message }) {
  if (!open) return null;

  const modalCls = [
    "relative w-full max-w-3xl mx-6 rounded-2xl p-10",
    "bg-surface border border-token",
    "shadow-[0_20px_60px_rgba(0,0,0,0.15)]",
    "dark:shadow-[0_20px_60px_rgba(0,0,0,0.45)]",
  ].join(" ");

  const okBtnCls =
    "h-9 px-10 rounded-md bg-[#2F8DCD] text-white font-semibold shadow " +
    "hover:brightness-95 active:translate-y-[1px] transition";

  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      {/* Backdrop */}
      <div
        className="absolute inset-0 bg-black/30 dark:bg-black/55"
        onClick={onClose}
      />

      {/* Modal */}
      <div className={modalCls} role="dialog" aria-modal="true">
        <div className="text-center font-semibold text-main">{message}</div>

        <div className="mt-6 flex justify-center">
          <button onClick={onClose} className={okBtnCls} type="button">
            OK
          </button>
        </div>
      </div>
    </div>
  );
}
